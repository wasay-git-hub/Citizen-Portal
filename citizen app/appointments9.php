<?php
  ...
  $sql_for_retrieval = "SELECT * FROM passport_applicants WHERE cnic = '$cnic_to_display'";
...
<head>
  <meta charset="UTF-8">
  <title>Passport Appointments</title>
  <style>
    body {
      margin: 0;
...
<body>
    ...
    <h2>Appointments for Passport</h2>
    <?php
    if(mysqli_num_rows($result)>0){
        for($i =1; $i<=mysqli_num_rows($result); $i++){
            ...
            ${"retrieved_address$i"} = $row['address'];
            ${"retrieved_date_allotted$i"} = $row['date_allotted'];
            ${"retrieved_status$i"} = $row['status'];
            ${"retrieved_category$i"} = $row['category'];
            ${"retrieved_nationality$i"} = $row['nationality'];
  ?>
    <div class="container">
    <div class="card">
      ...
      <p><strong>Nationality:</strong><?php echo" ${"retrieved_nationality$i"}" ?></p>
      <p><strong>Category:</strong><?php echo" ${"retrieved_category$i"}" ?></p>
  ...
  ?>
  <p id="xyz">You haven't applied for Passport.</p>
  <?php
    }
  ?>
</body>
